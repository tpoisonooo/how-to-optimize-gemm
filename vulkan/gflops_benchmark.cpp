...
void kompute(const std::string &shader) {
  ...
  auto algorithm =
      mgr.algorithm(params,
                    ...
                    compileSource(shader), workgroup, specConsts, pushConstsA);
  ...
  mgr.sequence()
      ...
      ->record<kp::OpAlgoDispatch>(algorithm,
                                   pushConstsB) // Overrides push consts
  ...
  for (const float &elem : tensorOutA->vector())
    std::cout << elem << "  ";
  // Prints the second output which is: { 10, 10, 10 }
  for (const float &elem : tensorOutB->vector())
    std::cout << elem << "  ";
